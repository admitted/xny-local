...
table{
        table-layout:fixed;word-wrap:break-word;white-space:nowrap;
}
</style>
...
<div id="down_bg_2">
	<table width="100%" style='margin:auto;' border=0 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
		<tr height='25px' class='sjtop'>
	         ...
	         &nbsp;年份月份:<select name="Year" style="width:70px;height:20px;" onChange="doSelect(this.value)">
		        <%
		...
		<tr height='30' valign='middle'>
			<td width='100%' align='center' colspan=2>
				<table style="width:100%;" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff"  >
					<tr height='30px'>
							<td  width='3%' align=center class="table_deep_blue" ><strong>站\日</strong></td>
				<%
				  for(int i = 1; i<= thatMonthDays ;i++){
				%>
						  <td  width='30px'  align=center class="table_deep_blue" ><strong><%=i%></strong></td>
				<%
					}
        %>
				  </tr>
			  ...
			  %>
					<tr height='30' <%=((sn%2)==0?"class='table_blue'":"class='table_white_l'")%>>
								<td width='3%' align=center ><strong><%=statBean.getBrief()%></strong></td>
				<%
					  for(int i = 1; i<= thatMonthDays ;i++)
				    {
				%>
						    <td width='30px'  align=center><strong><%=CpmDaysMap.get(i) == null ?"0":CpmDaysMap.get(i)%></strong></td>		  
				<% 
					  }
				%>
				  </tr>		  
...
function doExport()
{
  ...
  //{
		if(window.XMLHttpRequest)
	  {
		...
		var url = "doTables_Export.do?Sid=<%=Sid%>&BTime="+BTime+"&ETime="+ETime+"&Month="+doTables.Month.value;
